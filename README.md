...
- **代码的严谨程度我没有能力负责。** 大部分实现是我和 AI 一起完成的，测试写了 503 条、也逐个跑通了，但那不等于没有设计缺陷或安全漏洞。**装之前请自己审计一遍代码**，尤其是 `src/relay.js`（对外的 HTTP/WebSocket 入口）和 `src/extension.js`（权限响应逻辑）。文件不多，全部只用 Node 标准库，没有第三方依赖，读起来不费劲。
...
- **一致性门禁** —— 503 个测试，含审批链路端到端验证与 UI 一致性检查（DOM id / CSS class / 前后端消息类型双向对齐 / 解析器 kind 与渲染分支对齐）
...
**从手机新建会话时，只能选 Bridge 所在那个窗口的工作区 —— 选别的窗口的，agent 实际仍会在 Bridge 所在窗口的工作区里干活。**

会话本身确实建在你选的工作区下（`session/new` 发给了那个窗口的连接），但紧接着必须让桌面面板打开它、否则它的审批请求会在 6~20 毫秒内被判 cancelled，手机上根本批不动。而"打开面板"走的是 VS Code 命令，**只能在扩展自己所在的那个窗口执行**，扩展宿主是一个窗口一个进程，命令跨不了窗口。于是那个窗口就接管了这个会话，连 agent 看到的工作区上下文一起接管。

实测：手机上选 Kiro 工作区建的会话，磁盘上两个 workspace 目录下各有一份 `session.json` —— 目标窗口那份带着正确的模型却没有任何消息，Bridge 所在窗口那份有全部消息，而消息里出现的路径全是后者工作区下的。

**0.7.9 起，跨窗口创建时手机上会弹一个提示说明这件事**（不是 toast，因为它会静默产出错误的工作成果，值得打断你一次）。要真在另一个工作区里干活，得在电脑上把 Bridge 切到那个窗口再建会话。

根治需要让「拥有会话的那个窗口」自己去打开面板，也就是给非主窗口的扩展实例加一条从属通道连回主实例。那是个独立改动，还没做。
...
```bash
node test/run-all.mjs        # 全量测试（503 项）
...
- 想直接改的话，PR 我都会看。本地跑一下 `node test/run-all.mjs` 确认 503 条全绿即可，没有别的要求。
