...
- **代码的严谨程度我没有能力负责。** 大部分实现是我和 AI 一起完成的，测试写了 536 条、也逐个跑通了，但那不等于没有设计缺陷或安全漏洞。**装之前请自己审计一遍代码**，尤其是 `src/relay.js`（对外的 HTTP/WebSocket 入口）和 `src/extension.js`（权限响应逻辑）。文件不多，全部只用 Node 标准库，没有第三方依赖，读起来不费劲。
- **这个服务会打开一个能驱动你 agent 的入口。** 拿到访问 URL 的人可以看你全部会话历史、发消息、批准工具调用 —— 也就是能让你的机器执行命令。请务必读完[安全说明](#安全说明)再决定。
- **它依赖 Kiro 未公开的内部接口，随时可能失效。** 手机端能批准工具调用，靠的是 Kiro 内部的一个多路复用通道和一个未文档化的方法 —— 那些都不是对外承诺的接口。**Kiro 升级之后这个项目可能整体不工作，而且不会有任何提前通知。** 我会尽量跟进，但修不修得动、多久能修好，我都没法保证。装之前请接受这个前提。
...
- **一致性门禁** —— 536 个测试，含审批链路端到端验证与 UI 一致性检查（DOM id / CSS class / 前后端消息类型双向对齐 / 解析器 kind 与渲染分支对齐）
...
| `autoStart` | `false` | IDE 启动后自动开启远程会话。多窗口时只有一个窗口能占到端口，其余窗口会提示「端口已被占用，本窗口不再重复开一个服务」——**这不是故障**：占到端口的那个 relay 会连上全部窗口的 mux 端点，所有窗口的会话在手机上都能操作（实测 4 窗口场景下 4 个端点全部连上）。只有一个 Bridge 就够用 |
...
**从手机给别的窗口的工作区新建会话，需要那个窗口里也开过一次远程会话。**

0.8.0 起支持了：会话建在你选的工作区下，然后由**那个窗口自己**打开它，所以 agent 的工作区上下文是对的。

机制是这样的：新建会话之后必须让桌面面板打开它，否则它的审批请求会在 6~20 毫秒内被判 cancelled，手机上根本批不动。而"打开面板"走 VS Code 命令，**只能在扩展自己所在的窗口执行**（扩展宿主一个窗口一个进程）。所以那些没抢到端口的窗口会连回主实例待命，主实例把"打开这个会话"派给负责该工作区的那个窗口去做。

**前提是目标窗口里有待命的实例。** 在那个窗口执行一次「开启远程会话」就会进入待命（它会提示端口已被占用，那是正常的）。没有待命实例时会退回旧行为（由 Bridge 所在窗口接管，agent 在那个工作区里干活），并且手机上会弹一个提示说明这件事 —— 不是 toast，因为它会静默产出错误的工作成果，值得打断一次。

<details>
<summary>0.7.9 及更早的行为（这个限制当时无法绕开）</summary>

那时 attach 只能发生在 Bridge 所在窗口，于是那个窗口必然接管会话与工作区上下文。实测：手机上选 Kiro 工作区建的会话，磁盘上两个 workspace 目录下各有一份 `session.json` —— 目标窗口那份带着正确的模型却没有任何消息，Bridge 所在窗口那份有全部消息，而消息里出现的路径全是后者工作区下的。
...
```bash
node test/run-all.mjs        # 全量测试（536 项）
...
- 想直接改的话，PR 我都会看。本地跑一下 `node test/run-all.mjs` 确认 536 条全绿即可，没有别的要求。
