...
- **代码的严谨程度我没有能力负责。** 大部分实现是我和 AI 一起完成的，测试写了 562 条、也逐个跑通了，但那不等于没有设计缺陷或安全漏洞。**装之前请自己审计一遍代码**，尤其是 `src/relay.js`（对外的 HTTP/WebSocket 入口）和 `src/extension.js`（权限响应逻辑）。文件不多，全部只用 Node 标准库，没有第三方依赖，读起来不费劲。
...
- **一致性门禁** —— 562 个测试，含审批链路端到端验证与 UI 一致性检查（DOM id / CSS class / 前后端消息类型双向对齐 / 解析器 kind 与渲染分支对齐）
...
| `autoStart` | `false` | IDE 启动后自动开启远程会话。**多窗口时建议开** —— 第一个抢到端口的窗口成为主实例，其余窗口会自动进入待命（提示「端口已被占用…已经连上它待命」，**这不是故障**）。待命的窗口有两个用处：手机上给它的工作区新建会话时由它自己托管（agent 才会在对的工作区里干活），以及主实例所在窗口关掉时它会自动接管。不开的话得在每个窗口手动执行一次「开启远程会话」，而待命状态在窗口重载后就没了 |
...
**前提是目标窗口里有待命的实例。** 最省事的办法是打开 `autoStart`（见配置表）——Kiro 一启动，第一个窗口成为主实例、其余自动待命，不用手点。否则要在每个窗口执行一次「开启远程会话」，而**待命状态在窗口重载后就没了**，得重新来一次。

没有待命实例时会退回旧行为（由 Bridge 所在窗口接管，agent 在那个工作区里干活），并且手机上会弹一个提示说明这件事 —— 不是 toast，因为它会静默产出错误的工作成果，值得打断一次。

**0.8.1 起，主实例所在的窗口关掉后，待命的窗口会自动接管**，手机上的地址和 token 都不变、不用重新扫码。所以只要还有一个待命窗口活着，远程访问就不会因为你关掉某个窗口而中断。
...
```bash
node test/run-all.mjs        # 全量测试（562 项）
...
- 想直接改的话，PR 我都会看。本地跑一下 `node test/run-all.mjs` 确认 562 条全绿即可，没有别的要求。
